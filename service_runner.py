...
class NVRAWindowsService(win32serviceutil.ServiceFramework):
    ...
    def main(self):
        # Setup Database and Supervisor
        db_mgr = DatabaseManager()
        
        # Path ke NVRA-Worker.exe yang berada di folder instalasi yang sama
        base_dir = os.path.dirname(sys.executable)
        worker_exe = os.path.join(base_dir, "NVRA-Worker.exe")
        if not os.path.exists(worker_exe):
            worker_exe = sys.executable  # Fallback python
        ...
        while self.is_running:
            ...
            # Wait for 3 seconds or stop signal
