...
class AutonomousTradingWorker:
    ...
    def _run_single_autonomous_cycle(self) -> None:
        ...
        # Real-trade lifecycle → training data: sync myTrades fills as source of truth
        try:
            syncer = TradeFillSync(
                self.state_mgr,
                self.exchange,
                exchange_id=getattr(self, "exchange_id", "TOKOCRYPTO"),
            )
            n_new = syncer.sync_symbols(syncer.symbols_from_journal_and_orders(), limit=100)
            if n_new:
                logger.info("TradeFillSync ingested %s new exchange trades", n_new)
        except Exception as sync_err:
            logger.error("TradeFillSync failed (swallowed): %s", sync_err)
