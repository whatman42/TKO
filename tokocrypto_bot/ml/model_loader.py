...
2. Repo / frozen / CWD models/champion_model.pkl
...
def resolve_model_path() -> Optional[Path]:
    ...
    2. Repository / frozen bundle / CWD models/
    3. Platform-specific app data directory
    ...
    candidates = []
    candidates.append(Path(__file__).resolve().parent.parent.parent / "models" / MODEL_FILENAME)
    if getattr(sys, "frozen", False):
        meipass = getattr(sys, "_MEIPASS", None)
        if meipass:
            candidates.append(Path(meipass) / "models" / MODEL_FILENAME)
        candidates.append(Path(sys.executable).resolve().parent / "models" / MODEL_FILENAME)
    candidates.append(Path.cwd() / "models" / MODEL_FILENAME)

    for candidate in candidates:
        try:
            if candidate.is_file():
                logger.info(f"Model resolved from path: {candidate}")
                return candidate
        except OSError:
            continue
    ...
    logger.warning(
        "No model found in any resolution location. Checked:\n"
        f"  1. NVRA_MODEL_PATH={env_model_path}\n"
        f"  2. Candidates: {candidates}\n"
