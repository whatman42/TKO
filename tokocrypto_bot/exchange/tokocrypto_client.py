...
class TokocryptoDirectClient:
    ...
    def post_stop_loss_limit_non_retry(self, symbol: str, side: str, quantity: float, stop_price: float, limit_price: float = None, client_order_id: str = None):
        """Single-shot STOP_LOSS_LIMIT protective POST — no automated retry."""
        import time, hmac, hashlib
        endpoint = f"{self.base_url}/open/v1/orders"
        params = {
            "symbol": symbol,
            "side": side,
        ...
        # reuse signing if present
        if hasattr(self, "_sign"):
            params = self._sign(params)
        res = self.session.post(endpoint, data=params, timeout=8.0)
        if res.status_code == 429:
            raise RateLimitError("POST STOP_LOSS_LIMIT rate-limited (429) — no retry")
        res.raise_for_status()
