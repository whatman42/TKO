...
class MLJournal:
    ...
    def fetch_training_rows(
        ...
        *,
        ...
        """Return training rows ordered by feature_timestamp.

        CL-1 default: pnl_only=True → label only from realized_pnl_usdt
        (1 if pnl>0 else 0). Weak BUY/SELL labels only if allow_weak_labels=True.
        """
        try:
            conn = self.db.get_connection()
            try:
                sql = """
                    SELECT p.features_json, p.decision_action, p.prediction_valid,
                           o.realized_pnl_usdt, o.side, p.feature_timestamp,
                           p.feature_version, o.outcome_status
                ...
                params: list = []
                if feature_version:
                    sql += " AND p.feature_version = ?"
                    params.append(str(feature_version))
                sql += " ORDER BY p.feature_timestamp ASC"
                rows = conn.execute(sql, tuple(params)).fetchall()
            finally:
                conn.close()
            out = []
            for features_json, decision_action, _valid, pnl, side, fts, fver, ostatus in rows:
                try:
                ...
                if pnl is not None:
                    try:
                        ...
                        label = None
                elif allow_weak_labels and not pnl_only:
                ...
                if label is None:
                    continue
                if pnl_only and pnl is None:
                    continue
                out.append({
                    "features": feats,
                    "label": int(label),
                    "feature_timestamp": int(fts or 0),
                    "feature_version": str(fver or ""),
                    "realized_pnl_usdt": float(pnl) if pnl is not None else None,
                })
            return out
        except Exception as e:
