...
Version: 2026.08 (v5.1) — Codename: sea.py

FIXES:
- Menambahkan method `set_goal` pada class GodEntity (fix error missing attribute).
- Mempertahankan semua fitur v5.0.
- Siap digunakan sebagai modul autonomous evolution dalam pipeline IDX.

PRINCIPLE:
Kebebasan agent dengan prinsip tidak punah adalah mutlak.
Setiap evolusi selalu diterima, tetapi sistem memiliki self-healing dan rollback otomatis.
...
- Adaptive Research Engine: ddgs primary, SearXNG secondary, Wikipedia fallback, DDG HTML last resort
- Gemini model discovery & fallback
- Resource Manager
- Decision Engine (dengan anti-loop)
- Problem Solver (aktif di run_loop)
- Self-Healing Core
- Git mode: disabled | commit | push | autonomous
- Setiap kandidat evolusi selalu dipromosikan, lalu diverifikasi; jika rusak, rollback.
"""
...
CORE_FILENAME = "sea.py"   # ganti nama modul
GENERATION_PREFIX = "G"
DEFAULT_TIMEOUT = 300
...
class VerificationEngine:
    ...
    def verify(self, candidate_path: Path) -> Dict[str, Any]:
        ...
        # AST parse
        try:
            ast.parse(candidate_path.read_text(encoding='utf-8'))
        ...
        results["checks"]["py_compile"] = {"ok": code == 0, "stderr": err[:300]}

        # boot test timeout 120 detik
        code, out, err = safe_run(
            [sys.executable, str(candidate_path), "--test-boot"],
            timeout=120,
            cwd=self.base_dir,
        )
        results["checks"]["boot_test"] = {
            "ok": code == 0,
            "stdout_tail": out[-500:],
            "stderr_tail": err[-300:],
        }
        ...
        if shutil.which("pytest"):
            ...
            if code == 0:
                ...
                results["checks"]["pytest"] = {"ok": False, "output": "no tests ran"}
            else:
                results["checks"]["pytest"] = {"ok": False, "output": out[-500:]}
        else:
            results["checks"]["pytest"] = {"ok": False, "output": "pytest not available"}
...
class DecisionEngine:
    ...
    def choose_action(self, iteration: int) -> str:
        ...
        elif snap["load_1m"] > snap["cpu_count"] * 1.5:
            action = "EXPLORE_ENV"
        elif not snap["network"]:
            action = "OPTIMIZE_SELF"
        elif iteration % 3 == 0:
            action = "EVOLVE_FULL"
        elif iteration % 3 == 1:
        ...
        if self.repeat_count >= 2:
            action = "OPTIMIZE_SELF" if action == "EXPLORE_ENV" else "RESEARCH"
            self.repeat_count = 0  # reset
...
# =============================================================================
# EVOLUTION ENGINE (Setiap kandidat selalu dipromosikan, rollback jika rusak)
# =============================================================================
...
"""
    ...
    def promote(self, gen_id: str, candidate_path: Path, test_results: Dict[str, Any]) -> bool:
        """Selalu promosikan kandidat; backup dibuat; rollback jika gagal."""
        backup = self.core_path.with_suffix(".py.bak")
        shutil.copy2(self.core_path, backup)  # selalu backup
...
# =============================================================================
# GIT MANAGER (mendukung autonomous mode)
# =============================================================================

class GitManager:
    def __init__(self, root: Path, mode: str = GIT_MODE_DEFAULT):
        self.root = root
        self.mode = mode  # disabled | commit | push | autonomous
    ...
    def commit_if_enabled(self, message: str) -> bool:
        ...
        if self.mode in ("push", "autonomous"):
            remote_code, _, _ = safe_run(["git", "remote"], cwd=self.root)
            if remote_code == 0:
...
class GodEntity:
    ...
    def set_goal(self, goal: str) -> None:
        """Menetapkan goal saat ini dan menyimpannya ke memori."""
        self.goal = goal
    ...
    def plan(self, goal):
        plan = {
            "goal": goal,
            "tasks": ["Observe", "Research", "Hypothesis", "Candidate", "Verify", "Promote", "Self-heal"],
            "evaluation_criteria": ["syntax", "boot", "tests", "overall"],
        }
    ...
    def evolve_once(self):
        ...
        # Verifikasi untuk pencatatan, tetapi tidak menghalangi promosi
        ...
        if promoted:
            # Setelah promosi, cek kesehatan core; jika rusak rollback
            if not self.validator.verify(self.core_path).get("overall"):
                log.warning("⚠️ Core invalid after promotion, rolling back...")
                self.evolution.rollback(gid)
                promoted = False
            else:
        ...
        return {"generation": gid, "verification": verification, "promoted": promoted}
        ...
        while True:
            ...
            try:
                if action == "EVOLVE_FULL":
                    ...
                    if not evo.get("promoted"):
                        failure = {"category": "EVOLUTION", "context": evo.get("generation"), "error_type": "promotion_failed"}
                        self.failure_intel.record_failure(**failure)
                        self.problem_solver.solve(failure)
...
def build_parser():
    p = argparse.ArgumentParser(description="GOD ENTITY — REAL-WORLD EVIDENCE-DRIVEN SELF-EVOLUTION ENGINE (sea.py)")
